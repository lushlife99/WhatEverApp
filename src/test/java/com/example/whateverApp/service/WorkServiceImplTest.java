...
//        work = Work.builder().proceedingStatus(WorkProceedingStatus.CREATED).id(fakeWorkId).title("title").context("context").longitude(123.4).latitude(456.7).customer(mockUser).receiveLatitude(0.0).receiveLongitude(0.0).deadLineTime(1).build();
//    }
//
//
//    @Test
...
//
//        // When
//        Work work1 = workService.create(workDto, request);
//        WorkDto createdWorkDto = new WorkDto(work1);
//
//        // Then
...
//        // Given
//        when(jwtTokenProvider.getUser(request)).thenReturn(Optional.of(mockUser));
//        Work work1 = Work.builder().proceedingStatus(WorkProceedingStatus.STARTED).id(1L).createdTime(LocalDateTime.now()).finishedAt(LocalDateTime.now()).latitude(1.1).longitude(2.2).receiveLatitude(123.4).receiveLongitude(456.7).customer(mockUser).receiveLatitude(0.0).receiveLongitude(0.0).title("title").deadLineTime(1).context("context").build();
//        Work work2 = Work.builder().proceedingStatus(WorkProceedingStatus.CREATED).id(2L).createdTime(LocalDateTime.now()).finishedAt(LocalDateTime.now()).latitude(1.1).longitude(2.2).receiveLatitude(123.4).receiveLongitude(456.7).customer(mockUser).receiveLatitude(0.0).receiveLongitude(0.0).title("title").deadLineTime(1).context("context").build();
//        Work work3 = Work.builder().proceedingStatus(WorkProceedingStatus.CREATED).id(3L).createdTime(LocalDateTime.now()).finishedAt(LocalDateTime.now().plusHours(10)).latitude(1.1).longitude(2.2).receiveLatitude(123.4).receiveLongitude(456.7).customer(mockUser).receiveLatitude(0.0).receiveLongitude(0.0).title("title").deadLineTime(1).context("context").build();
//
...
//        assertEquals(resultDto.getProceedingStatus(), WorkProceedingStatus.STARTED);
//    }
//
//    @Test
