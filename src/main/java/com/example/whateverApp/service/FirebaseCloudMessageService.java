...
public class FirebaseCloudMessageService {
    ...
    public void sendMessageGroup(List<User> userList, String title, String body) throws FirebaseMessagingException {
        ...
        for (User user : userList) {
            ...
            notificationTokens.add(user.getNotificationToken());
        }

        MulticastMessage message = MulticastMessage.builder()
                .setNotification(Notification.builder().setTitle(title).setBody(body).build())
                .putData(MessageType.RouteType.getDetail(), RouteOptions.NEARBY_WORK_VIEW.getDetail())
        ...
        if (response.getFailureCount() > 0) {
            ...
            for (int i = 0; i < responses.size(); i++) {
                if (!responses.get(i).isSuccessful())
                    failedTokens.add(notificationTokens.get(i));
            }
